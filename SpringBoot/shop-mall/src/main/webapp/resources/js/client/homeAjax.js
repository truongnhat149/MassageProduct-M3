...
function addToCart(id)
	{
		$.ajax({
			type: "GET",		
			url: "http://localhost:8080/shopmall/api/cart/addProduct?id="+id,
...
$(document).ready(function(){
	...
	function ajaxGet(){
		$.ajax({
			type: "GET",		
			url: "http://localhost:8080/shopmall/api/products/latest",
			success: function(result){
				var content;
				...
				$.each(result, function(i, product){
					...
					if(i != result.length-1)
					{
						...
						{
							content = '';
							content = '<div class="grid_1_of_4 images_1_of_4 products-info"><a href="sp?id='+product.id+'">' +
									'<img style="width: 300px; height: 238px" src="/shopmall/img/'+product.id+'.png">' +
									'<h3 style="font-weight: bold;">'+product.nameProduct+'</h3></a>' +
									'<h3>'+accounting.formatMoney(product.unitPrice)+' VND</h3>'+
						...
						{
							content = content+'<div class="grid_1_of_4 images_1_of_4 products-info"><a href="sp?id='+product.id+'">' +
									'<img style="width: 300px; height: 238px" src="/shopmall/img/'+product.id+'.png">' +
									'<h3 style="font-weight: bold;">'+product.nameProduct+'</h3><a/>' +
									'<h3>'+accounting.formatMoney(product.unitPrice)+' VND</h3>'+
									'<button onClick="addToCart('+product.id+')" class="btn btn-warning"><span class= "glyphicon glyphicon-shopping-cart pull-center"></span> Giỏ hàng</button>'+
