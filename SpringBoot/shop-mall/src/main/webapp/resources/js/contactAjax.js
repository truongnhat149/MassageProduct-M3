$(document).ready(function() {
	...
	function ajaxGet(page){		
		...
		$.ajax({
			...
			url: "http://localhost:8080/shopmall/api/contact/all" + '?page=' + page,
			success: function(result){
				$.each(result.content, function(i, lienHe){
					var lienHeRow = '<tr>' +
					                  '<td>' + lienHe.id+ '</td>' +
					                  '<td>' + lienHe.emailContact + '</td>' +
					                  '<td>' + lienHe.ngayContact + '</td>' +
...
//		event.preventDefault();
	...
	$(document).on('click', '.btnChiTiet', function (event){
		...
		var href = "http://localhost:8080/shopmall/api/contact/"+lienHeId;
		$.get(href, function(lienHe) {
			$('#contentContact').html("<strong>Nội dung liên hệ</strong>: <br> "+ lienHe.contentContact);
			$('#ngayContact').html("<strong>Ngày liên hệ</strong>: "+ lienHe.ngayContact);
			$('#email').html("<strong>Email liên hệ</strong>: "+ lienHe.emailContact);
	...
	$(document).on('click', '.btnTraLoi', function (event){
		...
		var href = "http://localhost:8080/shopmall/api/contact/"+lienHeId;
		$.get(href, function(lienHe) {
			$('#ndLienHe').val(lienHe.contentContact);
			$('#emailContact').val(lienHe.emailContact);
		});
	...
	function ajaxPostReply(){
    	...
    	var formData = { id : $('input[name=lienHeId]').val(),
    	                 ...
    	                 tieuDe : "Trả lời câu hỏi:" + $('textarea[name=contentContact]').val(),
    			         diaChiDen: $('input[name=emailContact]').val()} ;
    	...
    	$.ajax({
			...
			type : "POST",
			contentType : "application/json",
			url : "http://localhost:8080/shopmall/api/contact/reply",
