			...
			success: function(result){
				$.each(result.content, function(i, danhMuc){
					var danhMucRow = '<tr style="text-align: center;">' +
				                      ...
				                      '</tr>';
					$('.categoryTable tbody').append(danhMucRow);
				});		
...
//    });
	...
	function ajaxPost(){
    	...
    	$.ajax({
			...
			url : "http://localhost:8080/shopmall/api/category/save",
    ...
    $(document).on("click",".btnCapNhatDanhMuc",function(){
		...
		var href = "http://localhost:8080/shopmall/api/category/"+danhMucId;
		$.get(href, function(danhMuc, status) {
			$('.updateForm #id').val(danhMuc.id);
			$('.updateForm #nameCategory').val(danhMuc.nameCategory);
		});
    ...
    function ajaxPut(){
    	...
    	$.ajax({
			...
			type : "PUT",
			contentType : "application/json",
			url : "http://localhost:8080/shopmall/api/category/update",
			data : JSON.stringify(formData),
            // dataType : 'json',
			success : function(response) {
    ...
    $(document).on("click",".btnXoaDanhMuc", function() {
		...
		if(confirmation){
		  $.ajax({
			  type : "DELETE",
			  url : "http://localhost:8080/shopmall/api/category/delete/" + danhMucId,
