$(document).ready(function() {
	...
	function ajaxGet(page){		
		var data = { nameRole:$("#vaiTro").val()};
		$.ajax({
			type: "GET",	
			data: data,
			contentType : "application/json",
			url: "http://localhost:8080/shopmall/api/account/all" + '?page=' + page,
...
//					                  '<td><button class="btn btn-primary btnCapNhat" >Cập nhật</button></td>' + 
    ...
    function ajaxPostTaiKhoan() {    	
    	...
    	console.log(data);
    	 ...
    	 $.ajax({
 			...
 			type : "POST",
 			contentType : "application/json",
 			url : "http://localhost:8080/shopmall/api/account/save",
    ...
    $(document).on("click",".btnXoa", function() {
		...
		if(confirmation){
		  $.ajax({
			  type : "DELETE",
			  url : "http://localhost:8080/shopmall/api/account/delete/" + taiKhoanId,
