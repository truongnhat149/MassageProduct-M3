...
<tr class="cart_line" id="item${sanpham.id}">
		<div>
		<td>${loop.index+1}</td>
		<td><img src="/shopmall/img/${sanpham.id}.png" style="width: 150px" class="cart-img"></td>
		<td style="text-align: center; mergin-top: -52px;">
			<p class="cart_ten"><a href="<c:url value="/sp?id=${sanpham.id}" />">${sanpham.nameProduct}</a></p>
			<p class="cart_masanpham">Mã sản phẩm : <span>${sanpham.id}</span></p>
			<p class="">Bảo hành : ${sanpham.thongTinBaoHanh}</p>
		</td>
		<td class="covertPriceProduct">${sanpham.unitPrice}</td>
		<td>
			<input class="input_num_cart" type="number" value="${quanity[sanpham.id]}" min="1" onChange="changeQuanity(${sanpham.id},this.value,${sanpham.unitPrice})">
		</td>
		<td><b><span class="total" id="item${sanpham.id}_total" name="total_price">${sanpham.unitPrice*quanity[sanpham.id]}</span> VND</b></td>
		<td class="cart-img">
...
<tr>
		...
		<td colspan="4">
			<p class="cart_totalOrder">Tổng giá trị đơn hàng : <span id="ordertotal"></span> VND</p>
		</td>
</tr>
