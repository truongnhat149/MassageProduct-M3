	...
	<div class="container">
		<div class="row">
			...
			<div class="col-md-10">
				...
				<h3 style="line-height: 2;">
					<span style="font-weight: bold">Số điện thoại: </span>${user.getphone()}</h3>
				<h3 style="line-height: 2;">
					...
					<c:forEach var="order" items="${list}" varStatus="loop">
						<tr style="text-align: center;">
							...
							<%-- 							<c:set var="tongGiaTri"
								value="${tongGiaTri + chiTiet.soLuongNhanHang*chiTiet.unitPrice}" /> --%>
							<td><c:forEach var="chiTiet"
									items="${order.danhSachChiTiet}">
									<p>
										<a href='<c:url value="/sp?id=${chiTiet.sanPham.id}" />'>${chiTiet.sanPham.nameProduct}</a><br>
									</p>

									<c:choose>
										<c:when 
											test='${(order.statusOrder == "Đang chờ giao") || (order.statusOrder =="Đang giao")  }'>

											<p>Số lượng: ${chiTiet.soLuongDat }</p>
											<hr>
											<c:set var="tongGiaTri"
												value="${tongGiaTri + chiTiet.soLuongDat*chiTiet.unitPrice}" />
										</c:when>
										<c:otherwise >
