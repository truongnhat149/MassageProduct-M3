$(document).ready(function() {
	...
	function ajaxGet(page){
		...
		$.ajax({
			...
			url: "http://localhost:8080/massageshop/api/san-pham/all" + '?page=' + page,
			success: function(result){
				$.each(result.content, function(i, sanPham){
					var sanPhamRow = '<tr>' +
					                  '<td>' + '<img src="/massageshop/img/'+sanPham.id+'.png" class="img-responsive" style="height: 50px; width: 50px" />'+'</td>' +
					...
					var checkTenDanhMuc = (sanPham.danhMuc.tenDanhMuc.toLowerCase()).indexOf("massage".toLowerCase());
					sanPhamRow += ( checkTenDanhMuc != -1)?'<button class="btn btn-primary btnCapNhatmassage" >Cập nhật</button>':'<button class="btn btn-primary btnCapNhatOther" >Cập nhật</button>';
	...
	$('.massageModal, .otherModal').on('hidden.bs.modal', function(e) {
		e.preventDefault();
		$("#idDanhMucmassage, #idDanhMucKhac").val("");
		$("#idSanPhammassage, #idSanPhamKhac").val("");
    ...
    function ajaxPostmassage() {
    	// PREPATEE DATA
    	 ...
    	 $.ajax({
 			...
 			type : "POST",
 			contentType : "application/json",
 			url : "http://localhost:8080/massageshop/api/san-pham/save",
    ...
    $(document).on("click",".btnCapNhatmassage", function(event){
		...
		var href = "http://localhost:8080/massageshop/api/san-pham/"+sanPhamId;
		$.get(href, function(sanPham) {
			populate('.updatemassageForm', sanPham);
			$("#idDanhMucmassage").val(sanPham.danhMuc.id);
    ...
    $(document).on("click",".btnXoaSanPham", function() {
		...
		if(confirmation){
		  $.ajax({
			  async:false,
			  type : "DELETE",
			  url : "http://localhost:8080/massageshop/api/san-pham/delete/" + sanPhamId,
    ...
    $(document).on("click",".btnChiTiet", function() {
    	...
    	var href = "http://localhost:8080/massageshop/api/san-pham/"+sanPhamId;
		$.get(href, function(sanPham) {
			$('.hinhAnh').attr("src", "/massageshop/img/"+sanPham.id+".png");
